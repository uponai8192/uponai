// src/lib/voice/lead-notification.ts
//
// Shared lead-capture notification email, used by every voice-demo entry point
// (Retell web call + Jambonz web call). Fire-and-forget — a mail failure must
...
/** Send the branded lead email. Never throws — errors are logged. */
export function sendLeadNotification(lead: Lead): void {
  ...
  transport.sendMail({
    ...
    to: LEAD_EMAIL,
    subject: `New voice demo lead: ${lead.name} (${lead.company})`,
